...
class SerialCommand:
    ...
    def __init__(self, portx, bps=9600, timeout=None, bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE, stopbites=serial.STOPBITS_ONE):
        """
        设置使用的串口和波特率，已经超时时间
        :param portx: 要使用的串口
        :param bps:  要使用的波特率
    ...
    def bytesize(self, bytesize):
        if bytesize in [serial.EIGHTBITS, serial.FIVEBITS, serial.SEVENBITS, serial.SIXBITS]:
            ...
            raise ValueError("bytesize should be in [serial.EIGHTBITS, serial.FIVEBITS, serial.SEVENBITS, serial.SIXBITS]")
    ...
    def stopbites(self, stopbites):
        if stopbites in [serial.STOPBITS_ONE, serial.STOPBITS_TWO, serial.STOPBITS_ONE_POINT_FIVE]:
            ...
            raise ValueError("stopbites should in  [serial.STOPBITS_ONE,serial.STOPBITS_TWO,serial.STOPBITS_ONE_POINT_FIVE]")
    ...
    def openserial(self):
        ...
        """
        try:
            self.__serial = serial.Serial(self.portx, baudrate=self.bps, timeout=self.timeout, parity=self.parity, bytesize=self.bytesize, stopbits=self.stopbites)
        except:
            return -1
...
class SerialReceive(threading.Thread):
    ...
    def serialcommand(self, serialcommand):
        if isinstance(serialcommand, SerialCommand):
            ...
            raise TypeError(
                "serialcommand should be a instance of class SerialCommand")
